...
void * doNetworking(void * ClientDetail){
  ...
  while(1){
    ...
    char data[1024]; uint8_t cipher[1024];
    ...
    if(strcmp(data,"SEND") == 0){
      ...
      // memset(data,0,1024);
      // strcpy(data,"SEND");
      send(Client[id].sockID,"SEND",1024,0);

      //read & send hash value
      uint8_t hash[1024];
      read = recv(clientSocket,hash,1024,0);
      hash[read]='\0';
      send(Client[id].sockID,hash,1024,0);

      //read & send size of cipher
      read = recv(clientSocket,data,1024,0);
      data[read] ='\0';
      send(Client[id].sockID,data,1024,0);

      //read & send cipher
      read = recv(clientSocket,cipher,1024,0);
      cipher[read] = '\0';
      send(Client[id].sockID,cipher,1024,0);
